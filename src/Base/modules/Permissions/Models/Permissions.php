...
use Xditn\Base\modules\Permissions\Enums\MenuStatus;
...
class Permissions extends Model
{
    ...
    public function storeBy(array $data): ?Model
    {
        return DB::transaction(function () use ($data)
        {
            if ($data['actions'] ?? false) {
                ...
                try {
                    $actions = MModule::getControllerActions($parentMenu->module, $parentMenu->permission_mark);
    ...
    protected function addAction($model, mixed $parent): mixed
    {
        ...
        if ($this->where('module', $model->getAttribute('module'))->where(
            'permission_mark',
            $model->getAttribute('permission_mark')
        )->first()) {
            return false;
        }
